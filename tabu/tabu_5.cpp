...
using namespace std;
/*禁忌法求解TSP*/
/*找随机解的时候再多个随机解中找最优的（总是在多个随机解中找最优容易忽略掉最优解）.另外修改了文件格式*/
string filename;    //文件名
...
void init(int argc,char **argv){
    ...
    neighborNum = (argc >= 5) ? atoi(argv[4]) : 20;
...
void findNeighbor(int *initRoute,int *tempRoute){
    ...
    if(flag == 0){
        ...
        while(n1 < neighborNum){
            ...
            ran3 = rand() % 3; //用3种方法来找邻居

            if(ran3 == 0){//swap(x,i, j): swap the element at index i in permutation x with element at index j
                memcpy(localTempRoute,initRoute,sizeof(int) * cityNum);
                swap(localTempRoute[ran1],localTempRoute[ran2]);
                localTempLen = costFunction(localTempRoute);
                if(localTempLen < tempLen && !in_tabuList(localTempRoute)){
                    tempLen = localTempLen;
                    memcpy(tempRoute,localTempRoute,sizeof(int) * cityNum);
                }
		        ...
		        for(int k = ran2 + 1; k < cityNum; ++k){
		    	    localTempRoute[index++] = initRoute[k];
		        }
	    	    for(int k = ran1; k <= ran2; ++k){
		    	    localTempRoute[index++] = initRoute[k];
		        }
                localTempLen = costFunction(localTempRoute);
                if(localTempLen < tempLen && !in_tabuList(localTempRoute)){
